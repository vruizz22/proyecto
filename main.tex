...
\begin{flushleft}
	...
	Para implementar el modelo se requiere contar con el archivo \texttt{24.zip} que contiene los archivos necesarios para la ejecución del modelo, así como también se requiere \texttt{main.py}, \texttt{parametros/} y se necesita de las librerias
	\texttt{locale} y \texttt{pandas}. Además se requiere contar como parte de los resultados la visualización de la siguiente página \url{https://stupendous-genie-3fd288.netlify.app/}. A continuación se detalla el contenido de los archivos:
	...
	\textbf{parametros/:} Directorio que contiene los datos necesarios para la ejecución del modelo. Contiene los siguientes archivos:
	...
	Los gráficos de cada variable se encuentran en \texttt{resultados.xlxs}, en la hoja de cálculo de resultados. A continuación se presentan los resultados obtenidos para cada variable de decisión.
	...
	Los gráficos estan en la hoja $x_{mit}$ y los nombres para los gráficos son: \texttt{Valores de x para cargador tipo 1} y \texttt{Valores de x para cargador tipo 2}.
	\subsection{$y_{it}$}
	...
	Los gráficos estan en la hoja $y_{it}$ y los nombres para los gráficos son: \texttt{Estación 1} y \texttt{Estación 2}.
	
	\subsection{$z_{it}$}

	El gráfico es un poco complejo de leer, pero lo que representa es que la infraestructura una vez que fue instalada se preserva su existencia hasta el final de los periodos, ya que la variable toma el valor 1 en un t y luego se mantiene constante para todas las estaciones.
	Para facilitar la lectura de la variable $z_{it}$, se presenta la siguiente recurso dinámico \url{https://stupendous-genie-3fd288.netlify.app/} en el cual se puede observar la instalación de infraestructura por periodo.
	Los gráficos estan en la hoja $z_{it}$ y los nombres para los gráficos son: \texttt{Infraestructura presente por estación el tiempo}.
	
	\subsection{$a_{mt}$}

	En el gráfico se puede observar que hubo compras de cargadores en distintos periodos t, esto, además de demostrar que hubo un gasto en cargadores, se relacionan con las variables $x_{mit}$, que debería aumentar en alguna estación en un periodo donde haya compra ya que no hay almacenamiento, y con la variable $b_{mit}$, que debería también cuando se compran cargadores en un periodo $t$.
	Los gráficos estan en la hoja $a_{mt}$ y los nombres para los gráficos son: \texttt{Cantidad de cargadores comprados en cada periodo}.
	...
	Los gráficos estan en la hoja $b_{mit}$ y los nombres para los gráficos son: \texttt{Cantidad de Cargadores Tipo 1 Que Se Instalaron por Estación} y \texttt{Cantidad de Cargadores Tipo 2 Que Se Instalaron por Estación}.
	...
	Los gráficos estan en la hoja $d_{mit}$ y los nombres para los gráficos son: \texttt{Demanda a satisfacer cargador 1} y \texttt{Demanda a satisfacer cargador 2}.
	\subsection{$S_{mt}$}

	Como fue mencionado anteriormente en 5.4, no hubo almacenamiento de cargadores ya que el costo de almacenamiento era superior al incremiento por periodo del costo de compra.
	En el siguiente gráfico podemos ver que el comportamiento de la satisfacción de demanda para cada periodo t en en una estacion i para los 2 tipos de cargadores. En el siguiente gráfico podemos ver que la cantidad de cargadores almacenados de ambos tipos en cada periodo tiene una correcta relación con las cantidades que habían en los periodos previos y posteriores, cumpliendo nuestra restricción.
	Los gráficos estan en la hoja $S_{mt}$ y los nombres para los gráficos son: \texttt{Cantidad de Cargadores Almacenados por Periodo}.
