...
\begin{document}
	...
	\begin{flushleft}
		...
		\begin{itemize}
			\item $x_{im}$, una variable binaria que indica si se instala un centro de carga en la ubicación $i$ ($x_i = 1$) o no ($x_i = 0$).
			\item $C_{im}$, una variable continua que da la cantidad de cargadores por estación.
			\item $P_{mit}$, una variable que representa el precio para la cargador tipo $m$, para la ubicación $i$ en el periodo $t$.
		\end{itemize}
